...
void OBSCounter::writeCareerStats() {
    gameWrapper->Execute([this](...) mutable {
        ...
        for (CareerStatsWrapper::StatValue careerValue : careerStats) {
            ...
            careerStatAverage[i] = divide(careerStatTotal[i], careerStatTotal[Win] + careerStatTotal[Loss]);

            std::ofstream averageFile(fileLocation / "Career" / ("Average" + indexStringMapCareer[i] + ".txt"));
            averageFile << std::fixed << std::setprecision(decimalPlaces);
            averageFile << careerStatAverage[i];
            averageFile.close();
        }
        });
}